      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <label htmlFor="logo-upload" className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 cursor-pointer hover:bg-blue-700 transition-colors">
              <Upload size={20} />
              رفع الشعار
            </label>
            {settings.logo && <img src={settings.logo} alt="Logo" className="w-12 h-12 object-cover rounded" />}
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium mb-2">أيقونة المتجر (Favicon)</label>
          <div className="flex items-center gap-3">
            <input type="file" accept="image/*" onChange={(e) => handleFileUpload(e, 'favicon')} className="hidden" id="favicon-upload" />
            <label htmlFor="favicon-upload" className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 cursor-pointer hover:bg-blue-700 transition-colors">
              <Upload size={20} />
              رفع الأيقونة
            </label>
            {settings.favicon && <img src={settings.favicon} alt="Favicon" className="w-8 h-8 object-cover rounded" />}
          </div>
