...
export const generateProductDetailHTML = (product: Product, settings: StoreSettings, relatedProducts: Product[] = []): string => {
...
<body class="min-h-screen bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto px-4 py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ${settings.logo ? `<img src="logo.png" alt="Logo" class="w-10 h-10 object-cover rounded-lg shadow-md" />` : ''}
                    <div>
                    ...
                    ${productDetailSettings.showRating ? `
                        <div class="flex items-center gap-2 mb-4">
                            <div class="flex items-center">
                                ${[...Array(5)].map(() => '<svg class="w-5 h-5 text-yellow-400 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>').join('')}
                            </div>
            ...
            <div class="mt-16">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ${relatedProducts.slice(0, productDetailSettings.relatedProductsLimit).map(relatedProduct => `
                        <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 cursor-pointer group transform hover:-translate-y-2" onclick="window.open('product-${relatedProduct.id}.html', '_blank')">
                            <img src="images/${relatedProduct.id}.jpg" alt="${relatedProduct.name}" class="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300" />
                            <div class="p-4">
                                <h3 class="font-semibold text-lg mb-2 group-hover:text-blue-600 transition-colors">${relatedProduct.name}</h3>
    ...
    <script>
        ...
        // FAQ Toggle Function for related sections
