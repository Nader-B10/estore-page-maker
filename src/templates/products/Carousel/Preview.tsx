...
import { Container, Carousel, Card, Button, Badge, Alert } from 'react-bootstrap';
import { TemplateProps } from '../../TemplateTypes';
import { Product, ProductSectionData } from '../../../types/store';
import { ShoppingBag } from 'lucide-react';

const PreviewProductCard: React.FC<{ product: Product }> = ({ product }) => {
  return (
...
export const Preview: React.FC<TemplateProps> = ({ storeData, sectionKey }) => {
  if (!sectionKey || !['featuredProducts', 'bestSellers', 'onSale'].includes(sectionKey)) {
    return null;
  }
  ...
  const { title, subtitle, autoplay } = sectionConfig.data as ProductSectionData;
    ...
    <section className="py-5 bg-light">
      <Container>
        ...
        {sectionProducts.length > 0 ? (
          <Carousel indicators={false} interval={autoplay ? 3000 : null}>
